...
using EvaluateExpressions: ReplaceConstantSymbolS!
...
function ReadReactionsEntry!(name::SubString{String}, var::SubString{String},
    ...
    if (read_step ==1)
        ...
        K_expr = WriteRateCoefficientsToModule(rate_coeff_str,
            current_reaction, 0)
        current_reaction.rate_coefficient = K_expr #K_funct_list[input_r_id]
    ...
    try
        ...
        if flag == 0
            ...
            if reaction.case == r_wall_loss
                write(f_ReactionSet,
                    string("push!(K_funct_list, (temp, species, system) -> ",expr,")\n"))
            else
                write(f_ReactionSet,
                    string("push!(K_funct_list, (temp) -> ",expr,")\n"))
            end
        end
